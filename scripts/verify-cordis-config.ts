...
/**
 * External DSH plugins vendored for the Trellis profile. They ship their own
 * source/build conventions (often plain JS or a bespoke build script) and are
 * deliberately not part of the repository TypeScript project graph.
 */
const externalVendoredPackages = new Set([
...
function validateExampleResolution(): string[] {
  ...
  for (const packageName of localExamplePackages) {
    if (externalVendoredPackages.has(packageName)) continue
