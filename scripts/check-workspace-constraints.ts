...
const vendoredPackages = new Set([
  ...
  // External DSH plugins vendored for the Trellis profile. They keep their
  // upstream manifests and publish/build workflows and are not repository
  // release members.
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  } else if (releaseMemberDirectory.test(dir) && (manifest.name === undefined || !vendoredPackages.has(manifest.name))) {
  ...
  } else if (manifest.private !== true && (manifest.name === undefined || !vendoredPackages.has(manifest.name))) {
...
function checkWorkspaceProtocol(manifests: readonly WorkspaceManifest[]): string[] {
  ...
  for (const { dir, manifest } of manifests) {
    if (manifest.name !== undefined && vendoredPackages.has(manifest.name)) continue
