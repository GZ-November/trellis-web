...
<a id="ctxtrellisknowledge--trellisknowledge"></a>

### `ctx.trellisKnowledge` — `TrellisKnowledge`

Durable knowledge API used by Trellis tools and future interfaces. Source identity determines document ids, so importing the same URL or file refreshes one record instead of creating duplicates.
...
Source: [`packages/trellis/trellis/src/knowledge.ts:226`](../../packages/trellis/trellis/src/knowledge.ts)
