...
//   - images and text documents are deferred to the shipped composer (image
//     rail / inline Trellis document blocks); PDF/DOCX/XLSX and other binaries
...
function injectCss(): void {
  ...
  // Capability flag for the shipped composer: while this plugin is mounted
  // the composer defers non-native files to it instead of rejecting them.
  document.documentElement.dataset.dshFileUpload = '1'
...
const NATIVE_COMPOSER_TEXT_MEDIA = new Set([
  'application/json',
  'application/xml',
  'application/yaml',
  'text/yaml',
...
const NATIVE_COMPOSER_TEXT_EXTENSIONS = new Set([
  'md', 'markdown', 'txt', 'html', 'htm', 'json', 'csv', 'tsv', 'xml', 'yaml', 'yml', 'org',
])

/**
 * Files the shipped composer already owns: images ride the image-attachment
 * rail and text documents are inlined as Trellis document blocks. Global
 * drop/paste defers those to the composer to avoid duplicate intake; the
 * paperclip path still uploads every file explicitly.
 */
function isNativeComposerFile(file: File): boolean {
  const extension = file.name.includes('.') ? file.name.split('.').at(-1)?.toLowerCase() : undefined
  return file.type.startsWith('image/')
    || file.type.startsWith('text/')
    || NATIVE_COMPOSER_TEXT_MEDIA.has(file.type)
    || (extension !== undefined && NATIVE_COMPOSER_TEXT_EXTENSIONS.has(extension))
}
...
/** Global drag overlay + paste: drag folders or non-native documents anywhere
 * over the window, or paste them into the composer to attach. */
function DragOverlay({ attach }: { attach: (files: File[]) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const onDrop = (e: DragEvent): void => {
      ...
      void (async () => {
        ...
        const files = (await collectDroppedFiles(e.dataTransfer?.items ?? null))
          .filter(file => isNativeComposerFile(file) === false)
    ...
    const onPaste = (e: ClipboardEvent): void => {
      const files = filesFromClipboard(e).filter(file => isNativeComposerFile(file) === false)
      if (files.length > 0 && files.some((f) => f.type !== '')) {
        e.preventDefault()
        void attach(files)
      }
    }
