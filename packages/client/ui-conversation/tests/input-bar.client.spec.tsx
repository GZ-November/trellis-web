...
describe('image draft rail', () => {
  ...
  it('accepts a drop anywhere on the page under the full-page overlay', () => {
    ...
    expect(view.getByRole('status').textContent).toContain('拖入图片、文本或 PDF 文档即可添加')
  ...
  it('rejects unsupported document formats before image intake', () => {
    ...
    // Unsupported types are rejected as a whole batch before image intake.
    const files = [
    ...
    fireEvent.drop(document.body, { dataTransfer: { types: ['Files'], files, dropEffect: 'none' } })
    expect(addImages).not.toHaveBeenCalled()
    expect(view.getByRole('alert').textContent).toContain('目前支持图片以及 Markdown、TXT')
  })

  it('defers PDF drops to an external upload plugin without a composer toast', () => {
    delete document.documentElement.dataset.dshFileUpload
    document.documentElement.dataset.dshFileUpload = '1'
    try {
      const addImages = vi.fn(() => null)
      const { view } = bench({ addImages })
      const files = [new File([new ArrayBuffer(64)], 'paper.pdf', { type: 'application/pdf' })]
      fireEvent.drop(document.body, { dataTransfer: { types: ['Files'], files, dropEffect: 'none' } })
      expect(addImages).not.toHaveBeenCalled()
  ...
  it('shows the projected limits in the drop overlay desc line', () => {
    ...
    expect(view.getByRole('status').textContent).toContain('文本文档最大 1MB；图片最多 20 张，每张 5MB；其他文档自动上传整理')
  })
  ...
  it('turns a pasted text file into a Trellis document block', async () => {
    const addImages = vi.fn(() => null)
    const { textarea, shell } = bench({ addImages })
    const document = new File(['linked study notes'], 'network.md', { type: 'text/markdown' })
    fireEvent.paste(textarea, {
      clipboardData: {
        items: [{ kind: 'file', type: 'text/markdown', getAsFile: () => document }],
        getData: () => '',
      },
    })
    await waitFor(() => {
      expect(shell.snapshot.draft).toContain('[Trellis document]\nName: network.md')
    })
    expect(shell.snapshot.draft).toContain('linked study notes\n[/Trellis document]')
    expect(addImages).not.toHaveBeenCalled()
  })
