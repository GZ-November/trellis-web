...
/** Browser intake bound for user-authored text inserted into one draft. */
const MAX_TEXT_DOCUMENT_BYTES = 1_000_000
const TEXT_DOCUMENT_EXTENSIONS = new Set([
  'md', 'markdown', 'txt', 'html', 'htm', 'json', 'csv', 'tsv', 'xml', 'yaml', 'yml', 'org',
])
...
function isTextDocument(file: File): boolean {
  ...
  return file.type.startsWith('text/') || TEXT_DOCUMENT_MEDIA_TYPES.has(file.type)
    || (extension !== undefined && TEXT_DOCUMENT_EXTENSIONS.has(extension))
}

function trellisDocumentBlock(file: File, content: string): string {
  return [
    '[Trellis document]',
...
export function InputBar({
  ...
  const onPaste = (e: React.ClipboardEvent<HTMLTextAreaElement>): void => {
    ...
    if (files.length > 0) void intakeFiles(files)
    if (files.some(isTextDocument)) {
      e.preventDefault()
      return
    }
  ...
  const intakeFiles = useCallback(async (files: readonly File[]): Promise<void> => {
    if (files.length === 0 || keyboard === undefined || locked || machineBusy) return
    const documents = files.filter(isTextDocument)
    const images = files.filter(file => file.type.startsWith('image/'))
    const unsupported = documents.length + images.length !== files.length
    // With a full file-upload plugin mounted (Trellis profile), PDF/DOCX/…
    // paste and drop belong to that plugin; the composer only owns images
    // and inline text documents. Without it, keep the loud format rejection.
    const uploaderMounted = typeof document !== 'undefined' && document.documentElement.dataset.dshFileUpload === '1'
    if (unsupported && !uploaderMounted) {
      showToast(t('document.unsupportedType'))
      return
    }
    if (images.length > 0) intakeImages(images)
    if (unsupported) return
    if (documents.some(file => file.size > MAX_TEXT_DOCUMENT_BYTES)) {
      showToast(t('document.tooLarge'))
      return
    }
    if (documents.length === 0) return
    let contents: string[]
    try {
      contents = await Promise.all(documents.map(file => file.text()))
    } catch {
      showToast(t('document.readFailed'))
      return
    }
    const text = documents.map((file, index) => trellisDocumentBlock(file, contents[index] ?? '')).join('\n\n')
    const el = inputRef.current
    const selection = el === null ? { start: keyboard.snapshot.draft.length, end: keyboard.snapshot.draft.length } : selectionOf(el)
    keyboard.pasteBegin(text, selection)
    const caret = selection.start + text.length
    if (el !== null) restoreCaret(el, caret)
    keyboard.track(keyboard.snapshot.draft, caret)
  }, [intakeImages, keyboard, locked, machineBusy, showToast, t])

  // Whole-page file-drop intake: the listeners live on the document so a drop
  // anywhere over the window adds images or text documents, not only
