...
export function ConversationRoot({
  ...
  // Automatically connect to the default workspace when starting cold with no session yet.
  useEffect(() => {
    if (sessionId === undefined && workspaces.items.length > 0 && pendingWorkspaceId === undefined) {
      const firstWs = workspaces.items[0]
      if (firstWs) {
        setPendingWorkspaceId(firstWs.workspaceId)
        void selectWorkspace(firstWs.workspaceId).catch(() => {
          setPendingWorkspaceId(undefined)
        })
      }
    }
  }, [sessionId, workspaces.items, pendingWorkspaceId, selectWorkspace])
