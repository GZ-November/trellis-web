...
export function dropOverlayLabels(
  ...
  if (!accepting) return { title: t('document.dropBlocked') }
  return {
    title: t('document.dropTitle'),
    desc: limits === undefined ? undefined : t('document.dropDesc', { count: limits.count, size: limits.size }),
  }
}
