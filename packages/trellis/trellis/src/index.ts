  ...
  ctx.inject(['webServer'], (innerCtx) => {
    ...
    innerCtx.effect(() => webServer.register({
      ...
      handler: (_req: IncomingMessage, res: ServerResponse): void => {
        const html = [
          ...
          '<style>',
          'body{margin:0;background:#0f1a14;color:#e8f5ee;font-family:ui-sans-serif,system-ui,sans-serif;overflow:hidden}',
          'header{padding:14px 24px;border-bottom:1px solid rgba(255,255,255,.08);display:flex;gap:12px;align-items:center;position:relative;z-index:2}',
          'h1{font-size:18px;margin:0}',
          'svg{width:100vw;height:calc(100vh - 52px);display:block;cursor:grab}',
          ...
          '.label{fill:#e8f5ee;font-size:10px;pointer-events:none;user-select:none}',
          '</style></head><body>',
          '<header><h1>Trellis Knowledge Canvas</h1><span id="stats"></span><span style="margin-left:auto;font-size:12px;color:#9fc9b4">drag to pan · scroll to zoom · drag nodes</span></header>',
          '<svg id="graph"></svg>',
          '<script>',
          ...
          'nodes.forEach((n,i)=>{const r=Math.min(innerWidth,innerHeight)*0.32;pos.set(n.id,{x:cx+r*Math.cos(angle*i),y:cy+r*Math.sin(angle*i)});});',
          'let view={x:0,y:0,k:1};',
          'function apply(){world.setAttribute("transform",`translate(${view.x},${view.y}) scale(${view.k})`);}',
          'edges.forEach(e=>{const a=pos.get(e.source),b=pos.get(e.target);if(!a||!b)return;const line=document.createElementNS(NS,"line");line.setAttribute("x1",a.x);line.setAttribute("y1",a.y);line.setAttribute("x2",b.x);line.setAttribute("y2",b.y);line.setAttribute("class","edge");world.appendChild(line);});',
          'nodes.forEach(n=>{const p=pos.get(n.id);if(!p)return;const g=document.createElementNS(NS,"g");g.setAttribute("transform",`translate(${p.x},${p.y})`);const c=document.createElementNS(NS,"circle");c.setAttribute("r",8);c.setAttribute("class","node");c.addEventListener("click",()=>{fetch(`/api/trellis/document?id=${encodeURIComponent(n.id)}`).then(r=>r.json()).then(d=>{alert(d.data.title+"\\n\\n"+(d.data.summary||""));});});g.appendChild(c);const t=document.createElementNS(NS,"text");t.setAttribute("class","label");t.setAttribute("y",-12);t.setAttribute("text-anchor","middle");t.textContent=n.title.length>24?n.title.slice(0,24)+"…":n.title;g.appendChild(t);world.appendChild(g);g.addEventListener("mousedown",ev=>{ev.stopPropagation();const startX=ev.clientX,startY=ev.clientY,orig={x:p.x,y:p.y};const move=me=>{p.x=orig.x+(me.clientX-startX)/view.k;p.y=orig.y+(me.clientY-startY)/view.k;g.setAttribute("transform",`translate(${p.x},${p.y})`);};const up=()=>{window.removeEventListener("mousemove",move);window.removeEventListener("mouseup",up);};window.addEventListener("mousemove",move);window.addEventListener("mouseup",up);});});',
          'let pan=null;',
          'svg.addEventListener("mousedown",e=>{pan={x:e.clientX-view.x,y:e.clientY-view.y};});',
          'window.addEventListener("mousemove",e=>{if(!pan)return;view.x=e.clientX-pan.x;view.y=e.clientY-pan.y;apply();});',
          'window.addEventListener("mouseup",()=>{pan=null;});',
          'svg.addEventListener("wheel",e=>{e.preventDefault();const factor=e.deltaY>0?0.9:1.1;view.k=Math.min(3,Math.max(0.2,view.k*factor));apply();},{passive:false});',
          'apply();',
    ...
    innerCtx.effect(() => webServer.register({
      ...
      handler: (_req: IncomingMessage, res: ServerResponse): void => {
        const html = [
          ...
          '<a href="/trellis/workspace">🧩 Unified Workspace</a>',
    ...
    innerCtx.effect(() => webServer.register({
      ...
      handler: (_req: IncomingMessage, res: ServerResponse): void => {
        const html = [
          '<!doctype html>',
          '<html lang="en"><head><meta charset="utf-8"><title>Trellis Workspace</title>',
          '<style>',
          'html,body{margin:0;height:100%;overflow:hidden;background:#0f1a14;font-family:ui-sans-serif,system-ui,sans-serif}',
          '.split{display:flex;height:100vh}',
          '.pane{flex:1;min-width:0;display:flex;flex-direction:column}',
          '.pane+.pane{border-left:1px solid rgba(255,255,255,.12)}',
          '.bar{height:44px;display:flex;align-items:center;padding:0 16px;gap:10px;background:rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.08);color:#e8f5ee;font-weight:600}',
          '.bar a{color:#7ef0b0;text-decoration:none;font-size:13px}',
          'iframe{flex:1;border:0;width:100%}',
          '</style></head><body>',
          '<div class="split">',
          '<div class="pane"><div class="bar">💬 Agent Chat <a href="/" target="_blank">open</a></div><iframe src="/" title="Trellis Chat"></iframe></div>',
          '<div class="pane"><div class="bar">🕸 Knowledge Canvas <a href="/trellis/canvas" target="_blank">open</a></div><iframe src="/trellis/canvas" title="Trellis Canvas"></iframe></div>',
          '</div></body></html>',
        ].join('\n')
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' })
        res.end(html)
      },
    }), 'trellis: /trellis/workspace route')
  })
}
