...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Durable knowledge API used by Trellis tools and future interfaces.',
    description: 'Durable knowledge API used by Trellis tools and future interfaces. Source identity determines document ids, so importing the same URL or file refreshes one record instead of creating duplicates.',
    methods: [
      {
        signature: 'diagnose(): string | undefined',
        description: 'Check package-owned graph relationships against the authoritative table.',
        parameters: [],
        returns: 'the first violated relationship, or `undefined` when consistent.',
      },
      {
        signature: 'async ingest(request: TrellisIngestRequest, signal?: AbortSignal): Promise<TrellisIngestResult>',
        description: 'Fetch or accept a source, validate every proposed target, then durably create or refresh its record. A failed fetch or relation validation writes nothing.',
        parameters: [{ name: 'request', description: 'source, organization metadata, and proposed relations.' }, { name: 'signal', description: 'cancellation forwarded to web retrieval.' }],
        returns: 'the stored record and whether it was newly created.',
      },
      {
        signature: 'search(query: string, limit?: number): TrellisSearchHit[]',
        description: 'Search titles, tags, summaries, and bodies with deterministic local ranking.',
        parameters: [{ name: 'query', description: 'non-blank search text.' }, { name: 'limit', description: 'requested hit count, capped by config.' }],
        returns: 'ranked compact hits.',
      },
      {
        signature: 'read(id: TrellisDocumentId): TrellisDocumentRead',
        description: 'Read one document, clipping only the returned body while retaining the durable record unchanged.',
        parameters: [{ name: 'id', description: 'document id.' }],
        returns: 'the bounded document and every inbound relation.',
      },
      {
        signature: 'async connect(sourceId: TrellisDocumentId, input: TrellisRelationInput): Promise<KnowledgeRelationRecord>',
        description: 'Create or replace one evidence-bearing directed relation. Both endpoints must already exist, and the source update is atomic.',
        parameters: [{ name: 'sourceId', description: 'document that owns the outgoing relation.' }, { name: 'input', description: 'target, semantics, evidence, and confidence.' }],
        returns: 'the stored relation.',
      },
      {
        signature: 'listAll(): KnowledgeDocumentRecord[]',
        description: 'List all stored documents with their metadata and relations.',
        parameters: [],
        returns: 'all stored document records sorted by newest update.',
      },
      {
        signature: 'graph(request: TrellisGraphRequest = {}): TrellisGraphSnapshot',
        description: 'Project a bounded graph from current records. A query selects matching documents followed by their direct neighbors; otherwise newest documents are selected.',
        parameters: [{ name: 'request', description: 'optional text filter and node limit.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'KnowledgeDocumentKind',
    declaration: 'export type KnowledgeDocumentKind = z.infer<typeof knowledgeDocumentKind>;',
  },
  {
    name: 'KnowledgeDocumentRecord',
    declaration: 'export type KnowledgeDocumentRecord = z.infer<typeof knowledgeDocumentRecord>;',
  },
  {
    name: 'KnowledgeRelationKind',
    declaration: 'export type KnowledgeRelationKind = z.infer<typeof knowledgeRelationKind>;',
  },
  {
    name: 'KnowledgeRelationRecord',
    declaration: 'export type KnowledgeRelationRecord = z.infer<typeof knowledgeRelationRecord>;',
  },
  {
    name: 'KnowledgeSource',
    declaration: 'export type KnowledgeSource = z.infer<typeof knowledgeSource>;',
  },
  {
    name: 'KvFacet',
  ...
  {
    name: 'TrellisDocumentId',
    declaration: 'export type TrellisDocumentId = Branded<\'TrellisDocumentId\'>;',
  },
  {
    name: 'TrellisDocumentRead',
    declaration: 'export interface TrellisDocumentRead {\n    readonly document: KnowledgeDocumentRecord;\n    readonly contentTruncated: boolean;\n    readonly backlinks: readonly TrellisGraphEdge[];\n}',
  },
  {
    name: 'TrellisGraphEdge',
    declaration: 'export interface TrellisGraphEdge {\n    readonly id: TrellisRelationId;\n    readonly source: TrellisDocumentId;\n    readonly target: TrellisDocumentId;\n    readonly kind: KnowledgeRelationKind;\n    readonly label?: string;\n    readonly evidence: string;\n    readonly confidence: number;\n}',
  },
  {
    name: 'TrellisGraphNode',
    declaration: 'export interface TrellisGraphNode {\n    readonly id: TrellisDocumentId;\n    readonly title: string;\n    readonly summary: string;\n    readonly kind: KnowledgeDocumentKind;\n    readonly tags: readonly string[];\n    readonly sourceLabel: string;\n}',
  },
  {
    name: 'TrellisGraphRequest',
    declaration: 'export interface TrellisGraphRequest {\n    readonly query?: string;\n    readonly limit?: number;\n}',
  },
  {
    name: 'TrellisGraphSnapshot',
    declaration: 'export interface TrellisGraphSnapshot {\n    readonly nodes: readonly TrellisGraphNode[];\n    readonly edges: readonly TrellisGraphEdge[];\n    readonly totalDocuments: number;\n    readonly truncated: boolean;\n}',
  },
  {
    name: 'TrellisIngestRequest',
    declaration: 'export interface TrellisIngestRequest {\n    readonly url?: string;\n    readonly content?: string;\n    readonly fileName?: string;\n    readonly mediaType?: string;\n    readonly title?: string;\n    readonly summary: string;\n    readonly kind?: KnowledgeDocumentKind;\n    readonly tags?: readonly string[];\n    readonly relations?: readonly TrellisRelationInput[];\n}',
  },
  {
    name: 'TrellisIngestResult',
    declaration: 'export interface TrellisIngestResult {\n    readonly created: boolean;\n    readonly document: KnowledgeDocumentRecord;\n}',
  },
  {
    name: 'TrellisRelationId',
    declaration: 'export type TrellisRelationId = Branded<\'TrellisRelationId\'>;',
  },
  {
    name: 'TrellisRelationInput',
    declaration: 'export interface TrellisRelationInput {\n    readonly targetId: TrellisDocumentId;\n    readonly kind: KnowledgeRelationKind;\n    readonly label?: string;\n    readonly evidence: string;\n    readonly confidence: number;\n}',
  },
  {
    name: 'TrellisSearchHit',
    declaration: 'export interface TrellisSearchHit {\n    readonly id: TrellisDocumentId;\n    readonly title: string;\n    readonly summary: string;\n    readonly kind: KnowledgeDocumentKind;\n    readonly tags: readonly string[];\n    readonly snippet: string;\n    readonly score: number;\n    readonly source: KnowledgeSource;\n}',
  },
  {
